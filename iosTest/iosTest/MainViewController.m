...
-(void)initPagingController:(NSString *)entitiesName
{
    ...
    NSLog(@"numberOfPage %d",numberOfPage);
...
-(IBAction)changePage:(id)sender
{
    ...
    {
        pagedTableViewDataSource = [coreDataManager loadContentsForEntity:entitiesNameOfMyFiles:pageFactor];
    }
    else
    {
        pagedTableViewDataSource = [coreDataManager loadContentsForEntity:entitiesNameOfSharedFiles:pageFactor];
    }
...
-(IBAction)fileSegmentChanged:(id)sender
{
    ...
    if (fileSegmentControl.selectedSegmentIndex == 0)
    {
        fileTableViewDataSource = myFileDrive.myFileConentsArray;
        pagedTableViewDataSource = [myFileDrive.myFileConentsArray mutableCopy];
        [self initPagingController:entitiesNameOfMyFiles];
        NSLog(@"0 selected");
    }
    else if (fileSegmentControl.selectedSegmentIndex == 1)
    {
        fileTableViewDataSource = myFileDrive.sharedFileConentsArray;
        pagedTableViewDataSource = [myFileDrive.sharedFileConentsArray mutableCopy];
